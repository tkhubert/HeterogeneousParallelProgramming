                                     ...
                                     int numCColumns) 
{
  ...
  {
	  int ColA = t*TILE_WIDTH+tx;
	  int RowB = t*TILE_WIDTH+ty;
	  bool checkA = (Row<numARows  && ColA<numAColumns);
  	  bool checkB = (RowB<numBRows &&  Col<numBColumns);
  	  A_t[ty][tx] = checkA ? A[Row*numAColumns+ColA] : 0;
  	  B_t[ty][tx] = checkB ? B[RowB*numBColumns+Col] : 0;
  	  __syncthreads();	
	
  	  for (int i=0; i<TILE_WIDTH; ++i)
	  	  val+= A_t[ty][i]*B_t[i][tx];
	  __syncthreads();
  }
...
int main(int argc, char **argv) {
  ...
  dim3 blocks ((numCColumns-1)/TILE_WIDTH+1, (numCRows-1)/TILE_WIDTH+1, 1);
